...
export default function App() {
  ...
  useEffect(() => {
    async function load() {
      const keys = ["pgbank","pgdeposits","pggames","pg_pix_config","pg_settings", ADMIN_PASSWORD_KEY];
      for (const k of keys) {
        try {
          const r = await window.storage.get(k, true);
          if (!r) continue;
          if (k === "pgbank")          setBank(parseFloat(r.value) || 0);
          else if (k === "pgdeposits") setDeposits(JSON.parse(r.value) || []);
          else if (k === "pggames")    { const g = JSON.parse(r.value); if (g?.length) setGames(g); }
          else if (k === "pg_pix_config") setPixConfig(p => ({ ...p, ...JSON.parse(r.value) }));
          else if (k === "pg_settings")   setSettings(s => ({ ...s, ...JSON.parse(r.value) }));
          else if (k === ADMIN_PASSWORD_KEY) setAdminPass(r.value);
        } catch {}
      }
    }
    load();
  ...
  async function persist(key, val) {
    try { await window.storage.set(key, typeof val === "string" ? val : JSON.stringify(val), true); } catch {}
  }
